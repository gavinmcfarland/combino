	...
	function compareFiles(
		...
		try {
			...
			if (stat1.isFile() !== stat2.isFile()) {
				differences.push(
					`${relativePath}: One is file, other is directory`,
				);
				return;
			}
			...
			if (stat1.isFile()) {
				...
				if (isTextFile) {
					...
					if (isJsonFile && options.parseJson) {
						try {
							...
							if (
								...
								differences.push(
									`${relativePath}: JSON content differs`,
								);
							}
							return;
						} catch (error) {
							differences.push(
								`${relativePath}: Invalid JSON - ${error}`,
							);
							return;
						}
					}
					...
					if (stat1.size !== stat2.size) {
						differences.push(
							`${relativePath}: File sizes differ (${stat1.size} vs ${stat2.size})`,
						);
					}
				}
			} else {
			...
			differences.push(
				`${relativePath}: Error comparing files - ${error}`,
			);
		}
	}

	function compareDirectoriesRecursive(
		dir1Path: string,
		dir2Path: string,
		relativePath: string,
	): void {
		try {
			...
			if (items1.length !== items2.length) {
				differences.push(
					`${relativePath}: Different number of items (${items1.length} vs ${items2.length})`,
				);
				return;
			}
			...
			for (let i = 0; i < items1.length; i++) {
				...
				if (options.ignoreExtensions && !namesMatch) {
					const name1WithoutExt = item1.substring(
						0,
						item1.lastIndexOf("."),
					);
					const name2WithoutExt = item2.substring(
						0,
						item2.lastIndexOf("."),
					);
				...
				if (!namesMatch) {
					differences.push(
						`${relativePath}: Item names differ ("${item1}" vs "${item2}")`,
					);
					continue;
				}
			...
			differences.push(
				`${relativePath}: Error reading directory - ${error}`,
			);
		}
	}
