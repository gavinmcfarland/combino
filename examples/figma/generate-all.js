...
async function addSourceFileComments(outputDir, templates) {
	console.log(chalk.gray('   🔍 Adding source file tracking...'));
			...
			if (sourceTemplate) {
				...
				switch (ext) {
					...
					case '.json':
						...
						console.log(chalk.gray(`     📄 ${relativePath} <- ${relativeSourcePath}`));
						break;
					case '.md':
						if (!content.includes('<!-- Source:')) {
							commentedContent = `<!-- Source: ${relativeSourcePath} -->\n\n${content}`;
				...
				if (commentedContent !== content) {
					...
					console.log(chalk.gray(`     ✏️  Added source comment to ${relativePath}`));
				}
			} else {
				// File not found in any template (might be generated by Combino)
				console.log(chalk.yellow(`     ⚠️  Could not find source for ${relativePath}`));
			}
		} catch (error) {
			// Ignore errors when adding comments
			console.log(chalk.yellow(`     ⚠️  Could not add source comment to ${file}: ${error.message}`));
		}
	}
}
