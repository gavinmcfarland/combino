...
export default function plugin(options: StripTSOptions = {}): Plugin {
	...
	const assemble = async (context: FileHookContext): Promise<FileHookResult | void> => {
		...
		// Skip vite-env.d.ts files and .d.ts files
		if (filename === 'vite-env.d.ts' || filename.endsWith('.d.ts')) {
			return;
		}
